...
def generate_ao_map(height_map, normal_map=None, radius=16, strength=1.0,
                    contrast=1.0, output_float=False):
    """
    ...
    Args:
        ...
        output_float (bool): Return float32 in [0, 1] instead of uint8.

    Returns:
        np.ndarray: Grayscale AO map, either float32 [0, 1] or uint8 [0, 255].
    """
    if len(height_map.shape) == 3:
        height_map = cv2.cvtColor(height_map, cv2.COLOR_BGR2GRAY)
        
    # Accept both the internal float representation and exported 8/16-bit maps.
    h = height_map.astype(np.float32, copy=False)
    if h.size and h.max() > 1.0:
        h /= 65535.0 if h.max() > 255.0 else 255.0
    h = np.clip(h, 0.0, 1.0)
    ...
    scales = [max(1, radius // 4), max(1, radius // 2), radius]
    weights = [0.5, 0.3, 0.2]
    
    for scale, weight in zip(scales, weights):
        ksize = scale if scale % 2 == 1 else scale + 1
        # Wrap at the borders so AO remains seamless with the source texture.
        padded = cv2.copyMakeBorder(h, scale, scale, scale, scale, cv2.BORDER_WRAP)
        blurred_h = cv2.GaussianBlur(padded, (ksize, ksize), 0)[scale:-scale, scale:-scale]
    ...
    if normal_map is not None:
        # Normal maps generated by SEAMS are BGR, so Z is channel 0.
        normal = normal_map.astype(np.float32, copy=False)
        normal_scale = 65535.0 if normal.max() > 255.0 else 255.0
        nz = (normal[..., 0] / normal_scale) * 2.0 - 1.0
    ...
    if output_float:
        return ao.astype(np.float32, copy=False)
    return np.rint(ao * 255.0).astype(np.uint8)
