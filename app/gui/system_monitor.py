"""
System Monitor Widget — CPU/GPU/RAM status bar chips.
...
class _GpuPollThread(QThread):
    """Runs the (up to 2s, blocking) nvidia-smi query off the GUI thread.

    _get_gpu_percent() is a synchronous subprocess call; calling it directly
    from a QTimer.timeout slot -- as the status bar's 1.5s refresh used to --
    blocks the entire GUI on every tick for the whole session.
    """
    result_ready = pyqtSignal(object)

    def run(self):
        self.result_ready.emit(_get_gpu_percent())
...
class StatusBarMonitor(QWidget):
    ...
    def _refresh(self):
        ...
        if self._gpu_poll_thread is not None:
            if self._gpu_poll_thread.isRunning():
                return  # previous query still in flight; skip this tick
            self._gpu_poll_thread.deleteLater()
        self._gpu_poll_thread = _GpuPollThread(self)
        self._gpu_poll_thread.result_ready.connect(self._on_gpu_percent)
        self._gpu_poll_thread.start()

    def _on_gpu_percent(self, pct):
        if pct is not None and self._gpu:
            self._gpu.set_value(pct)

    def stop(self):
        self._timer.stop()
        if self._gpu_poll_thread is not None and self._gpu_poll_thread.isRunning():
            self._gpu_poll_thread.wait(2500)
